...
driver(int argc, char *argv[])
{
  ...
  for ( ; pit.valid(); ++pit) {
	...
	if ((switch_value("gcseliminate") != 0) ||
      ...
      //subexpr_elimination(pgm, pgm_basic_blocks,switch_value("p_avail"),switch_value("availexpr"),switch_value("gcseliminate"),switch_value("copyprop"));
    ...
    if (switch_value("ddtest"))
      {
	ddtest(pgm,switch_value("dds_flow"),switch_value("dds_output"),switch_value("dds_anti")
    ,switch_value("dda_flow"),switch_value("dda_output"),switch_value("dda_anti"));
